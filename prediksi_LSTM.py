...
gs_slic_data = pd.concat([data_btc.iloc[-281:].copy(),pd.DataFrame(test_inverse_predicted,columns=['Open_predicted','Close_predicted'],index=data_btc.iloc[-281:].index)], axis=1)
...
for i in range(-100,0):
  up_pred = loaded_model.predict(curr_seq)
